...
from questionnaire.models import Answer, Question, Response
from questionnaire.signals import questionnaire_completed
...
class ResponseForm(models.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        """ Expects a questionnaire object to be passed in initially """
        ...
        self.steps_count = len(self.questionnaire.questions.all())
        # add a field for each questionnaire question, corresponding to the question
        # type as appropriate.
        data = kwargs.get("data")
        for i, question in enumerate(self.questionnaire.questions.all()):
            is_current_step = i != self.step and self.step is not None
            if self.questionnaire.display_by_question and is_current_step:
                continue
            else:
    ...
    def _get_preexisting_response(self):
        ...
        if not self.user.is_authenticated:
            return None
        try:
            return Response.objects.get(user=self.user, questionnaire=self.questionnaire)
        except Response.DoesNotExist:
            LOGGER.debug(
                "No saved response for '%s' for user %s", self.questionnaire, self.user
            )
            return None
    ...
    def has_next_step(self):
        if self.questionnaire.display_by_question:
    ...
    def next_step_url(self):
        if self.has_next_step():
            context = {"id": self.questionnaire.id, "step": self.step + 1}
            return reverse("questionnaire-detail-step", kwargs=context)
        else:
            return None

    def current_step_url(self):
        return reverse(
            "questionnaire-detail-step", kwargs={"id": self.questionnaire.id, "step": self.step}
        )
    ...
    def save(self, commit=True):
        ...
        for field_name, field_value in list(self.cleaned_data.items()):
            if field_name.startswith("question_"):
                ...
                answer.save()
        questionnaire_completed.send(sender=Response, instance=response, data=data)
        return response
