...
class Questionnaire2Csv(Questionnaire2X):
    @staticmethod
    ...
    def questionnaire_to_x(self):
        ...
        csv.append(Questionnaire2Csv.line_list_to_string(header))
        for response in self.questionnaire.responses.all():
            line = Questionnaire2Csv.get_user_line(question_order, response)
            csv.append(Questionnaire2Csv.line_list_to_string(line))
        return "\n".join(csv)
